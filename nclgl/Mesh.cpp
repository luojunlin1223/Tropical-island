...
void Mesh::Draw(GLsizei count)
{
	...
	if (bufferObject[INDEX_BUFFER]) {//是否使用EBO
		...
		glDrawArraysInstanced(type, 0, numVertices,count);//0代表 the starting index in the enabled arrays.
	}
	glBindVertexArray(0);//解绑VAO
}
...
void Mesh::DrawSubMesh(int i, GLsizei count)
{
	if (i < 0 || i >= (int)meshLayers.size()) {
		return;
	}
	...
	if (bufferObject[INDEX_BUFFER]) {
		const GLvoid* offset = (const GLvoid*)(m.start * sizeof(unsigned int));
		glDrawElementsInstanced(type, m.count, GL_UNSIGNED_INT, offset,count);
	}
	else {
		glDrawArraysInstanced(type, m.start, m.count,count);	//Draw the triangle!
	}
	glBindVertexArray(0);
}
/**
